const bookXhttp = new XMLHttpRequest();
bookXhttp.open("GET", "book.json");
bookXhttp.send();
bookXhttp.onreadystatechange = function (event) {
  if (event.target.readyState == XMLHttpRequest.DONE) {
    const bookResult = JSON.parse(event.target.response);
    console.log(bookResult);
    makebookSlideHtml(bookResult);
  }
};
